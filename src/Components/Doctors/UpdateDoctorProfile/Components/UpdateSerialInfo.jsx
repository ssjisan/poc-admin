...
export default function UpdateSerialInfo({ forBelow676, serialInfo, setSerialInfo }) {
  const days = [
  ...
  return (
    <Stack gap="16px" sx={{ width: "100%" }}>
      ...
      <Stack gap="16px">
            ...
            <Stack direction={forBelow676 ? "column" : "row"} gap="16px">
              <Autocomplete
                ...
                onChange={(event, newValue) => {
                  setSerialInfo((prevInfo) => {
                    const newInfo = [...prevInfo];
                    newInfo[index].consultationDays = newValue.map((day) => day.id);
                    return newInfo;
                  });
                }}
        ...
        <Button variant="soft" onClick={handleAddSerialInfo} startIcon={<PlusIcon size={20} color="#00AE60"/>}>
          Add Serial Info
        </Button>
        </Stack>
      </Stack>
